...
phone reading with answer-first, ASD-STE100-inspired controlled English as a
general default—not strict compliance. Set `LHC_PHOTON_MARKDOWN=false` to use plain text. Replies over
...
- **Finder** — cross-host list with per-thread summaries (latest brief chunk summary), projected LHC view upper estimates (qualified when visibility pruning applies), sortable columns, host/directory/text filters, health dots, live refresh. Hosts: cc-lhc, pi-lhc, codex-lhc, t3code-lhc, and hermes (registry-less: scans `~/.hermes{,/profiles/<name>}/lhc/threads`)
...
- **Detail tabs** — overview (identity/volume/view/health), histogram (per-turn stacked token-estimate bars by message kind, compact-point marker, click-through), turns (keyboard-navigable turn browser with full message content), thread view (current-view band ribbon + the true post-compact live tail, with omitted archived history reported separately)
- **Measurement contract** — every displayed number's query, unit, boundary, and source class is documented in [`docs/MEASUREMENTS.md`](docs/MEASUREMENTS.md)
